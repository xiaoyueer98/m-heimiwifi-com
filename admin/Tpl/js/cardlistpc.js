(function($){
      ...
      getData("get","json",{'page':1,'size':15,'pctype':2},"admin.php?m=Card&a=getlist",1);
...
function getCondition(page) {
      ...
      var size = $("#size").val();
      var data={'page':page,'size':size,'sta':sta,'title':title,'pctype':2};
      // 查询当前销售代表的客户列表
      getData("get", "json", data, "admin.php?m=Card&a=getlist", page);
}
      ...
      jQuery.ajax({
                  ...
                  $(jsondata['data']).each(function(key){
                         ...
                         tmpl=
                             ...
                             +'<td>'+this.endTime+'</td>'
                             +'<td><a href="admin.php?m=Card&a=upd&id='+this.id+'">修改</a></td>'
                             +'<td><a href="admin.php?m=Card&a=del&id='+this.id+'">删除</a></td>'
                             if (this.status == 2)
                             tmpl += '<td id="up"'+key+'><a href="admin.php?m=Card&a=setup&id='+this.id+'">置发布</a></td>'
                             else
                             tmpl += '<td id="down"'+key+'><a href="admin.php?m=Card&a=setdown&id='+this.id+'">置下架</a></td>'
