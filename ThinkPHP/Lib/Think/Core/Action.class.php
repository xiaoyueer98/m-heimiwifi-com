...
{//类定义开始
   ...
   /**
    ...
    /**
     +----------------------------------------------------------
     * $url 如果需要判断登录，则未登录状态下跳转地址
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     */
    protected function _is_Loin(){
        $module_name = MODULE_NAME;
        $action_name = ACTION_NAME;
        $is_login = C('IS_LOGIN');
        $is_login_module = $is_login['controller'];
      
        if(in_array($module_name,$is_login_module) && !unserialize(stripcslashes($_COOKIE['userinfo']))){
            $is_login_action = $is_login[$module_name];
            if(!empty($is_login_action) && !in_array($action_name, $is_login_action)){
                setcookie("action",$action_name,time()+3600,"/");
                header("location:index.php?m=Member&a=loginhm");
            }
        }
    }    
    /**
     +----------------------------------------------------------
     * 判断用户是否登录 登录则返回true  未登录返回false
     +----------------------------------------------------------
     * @access product
     +----------------------------------------------------------
     */
    protected function user_is_login(){
            $return = false;
